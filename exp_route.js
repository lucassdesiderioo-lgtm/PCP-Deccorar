...
module.exports=function(app,db){
  ...
  app.post('/api/lote/upload', bigJson, async (req,res)=>{
    try{
      ...
      const ins=db.prepare("INSERT INTO lote (codigo,cor,buyer,city,nf,packId,venda,codes,srcfile,labelPage,danfePage) VALUES (?,?,?,?,?,?,?,?,?,?,?)");
      let novos=0,rep=0,semsku=0;
      db.transaction(()=>{ for(const o of orders){
        ...
        if(!o.sku) semsku++;
        ins.run(o.sku,o.cor,o.buyer,o.city,o.nf,o.packId,o.venda,JSON.stringify(o.codes||[]),fname,o.labelPage,o.danfePage); novos++;
      }})();
      res.json({ok:true,total:orders.length,novos,repetidas:rep,sem_sku:semsku});
